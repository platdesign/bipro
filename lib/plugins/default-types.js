...
module.exports = function(protocol, options) {
	...
	protocol.defineType('string', {
		compose(string, data, schema, protocol) {
			...
			let sizeType = protocol._getType(schema.sizeType || 'uint8');
			let size = string.length;

			return Buffer.concat([
				sizeType.compose(size, data, {}, protocol),
				Buffer.from(string, schema.encoding || 'utf8')
			])
		},
		parse(buf, ctx, schema, protocol) {
			let sizeType = protocol._getType(schema.sizeType || 'uint8');
			...
			let res = buf.toString(schema.encoding || 'utf8', ctx.offset, ctx.offset+size);
			ctx.offset += size;
			return res;
		}
	});
	...
	protocol.defineType('array', {
		compose(arr, data, schema, protocol) {
			...
			let itemType = protocol._getType(schema.itemType || 'uint8');

			return Buffer.concat([
				sizeType.compose(size, data, {}, protocol),
				Buffer.concat(arr.map(item => itemType.compose(item, data, {}, protocol)))
			]);
		...
		parse(buf, ctx, schema, protocol) {
			...
			for(let i = 0; i < size; i++) {
				...
				let val = itemType.match(buf, ctx, schema, protocol);
				if(val === null) {
					return null;
				}
				res.push(val);
